...
import com.newtaxiprime.driver.databinding.ActivityEarningsBinding
import com.newtaxiprime.driver.home.datamodel.EarningModel
import com.newtaxiprime.driver.home.interfaces.ApiService
import com.newtaxiprime.driver.home.interfaces.ServiceListener
import com.newtaxiprime.driver.home.paymentstatement.PaymentStatementActivity
...
class EarningActivity : CommonActivity(), ServiceListener {
    ...
    lateinit var barChart: ChartProgressBar

    lateinit var triphistorylayout: RelativeLayout

    lateinit var paystatementlayout: RelativeLayout

    lateinit var horizontalScrollView: RelativeLayout

    lateinit var weekly_fare: TextView

    lateinit var last_trip_amount: TextView

    lateinit var most_resent_payout: TextView

    lateinit var show_date: TextView

    lateinit var value_mid: TextView

    lateinit var value_bottom: TextView

    lateinit var value_top: TextView

    lateinit var chat_empty: TextView

    lateinit var before_search: TextView

    lateinit var next_search: TextView

    lateinit var tvTotalPay: TextView

    lateinit var driverEarningAmount: TextView

    lateinit var tv_triphistoryarrow: TextView

    lateinit var tv_paystatementarrow: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        triphistorylayout = binding.triphistorylayout
        triphistorylayout.setOnClickListener{
            tripHistoryLayout()
        }
        paystatementlayout = binding.paystatementlayout
        paystatementlayout.setOnClickListener{
            paystatementLayout()
        }

        horizontalScrollView = binding.horizontalScrollView

        weekly_fare = binding.weeklyFare

        last_trip_amount = binding.lastTripAmount

        most_resent_payout = binding.mostResentPayout
        show_date = binding.showDate

        value_mid = binding.valueMid

        value_bottom = binding.valueBottom

        value_top = binding.valueTop

        chat_empty = binding.chatEmpty

        before_search = binding.beforeSearch

        before_search.setOnClickListener{
            beforeSearch()
        }

        next_search = binding.nextSearch
        next_search.setOnClickListener{
            nextSearch()
        }

        tvTotalPay = binding.tvTotalPay

        driverEarningAmount = binding.driverEarnAmount

        tv_triphistoryarrow = binding.triphistoryarrow
        tv_paystatementarrow = binding.paystatementarrow
        ...
        if (!TextUtils.isEmpty(sessionManager.userType) && !sessionManager.userType.equals(
                "0",
                ignoreCase = true
