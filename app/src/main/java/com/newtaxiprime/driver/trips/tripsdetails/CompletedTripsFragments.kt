...
import com.newtaxiprime.driver.common.util.Enums.REQ_PAST_TRIPS
...
class CompletedTripsFragments : Fragment(), ServiceListener, PaginationAdapterCallback {
    ...
    override fun onCreateView(
        ...
        recyclerView = rootView.findViewById(R.id.past_recycler_view)
        ...
        adapter?.setOnItemRatingClickListner(object :
            CompletedTripsPaginationAdapter.onItemRatingClickListner {
    ...
    private fun getCompletedTripsFlow() {
        val allHomeDataCursor: Cursor =
            dbHelper.getDocument(Constants.DB_KEY_COMPLETED_TRIPS.toString())
    ...
    private fun getPastTrips() {
        ...
        apiService.getPastTrips(sessionManager.accessToken!!, currentPage.toString())
            .enqueue(RequestCallback(REQ_PAST_TRIPS, this))
    }
                ...
                if (jsonResp.isSuccess) {
                    ...
                    if (currentPage == 1) {
                        dbHelper.insertWithUpdate(
                            Constants.DB_KEY_COMPLETED_TRIPS.toString(),
                            jsonResp.strResponse
                        )
    ...
    private fun getTripDetailApi() {
        apiService.getTripDetails(sessionManager.accessToken!!, sessionManager.tripId!!)
